(function(){
  ...
  const tabManage = document.querySelectorAll('.tab-manage') // 连接 html 里面的元素
  ...
  document.querySelector('.submit-task').addEventListener('click',()=>{ // 事件监听器，监听点击事件
    const name = taskName.value, time = taskTime.value // 获取输入框的值
    tasksTodo.push({ // 将输入框的值存入数组 tasksTodo
      name: name,
      time: time
    })
    localStorage.setItem('tasksTodo', JSON.stringify(tasksTodo)) // 将数组 存入 localStorage
    if(!!time) ipcRenderer.send('setTaskTimer', time, encodeURIComponent(name)) // 如果有时间，就发送给主进程进行倒计时
    genTodo ()
    activeTab (1) // 切换到左侧的选择框：待办 还是 已完成
    activeContent (0)
  })

  
  function genTodo () { // 遍历 tasksTodo 数组中的每个任务对象，将任务名称和时间插入到 HTML 字符串中
    let todoHtml = ''
  ...
  function genFinished () { // 遍历 tasksFinished 数组中的每个任务对象
    let finishHtml = ''
    ...
    taskFinished.innerHTML = finishHtml // 将任务名称和时间插入到 HTML 字符串中
    keepTimesDom.innerHTML = keepTimes // 坚持了几次
  }
