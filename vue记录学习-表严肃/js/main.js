/* myStorage.js */
;(function () { // 设置一个立即执行函数，防止变量污染
    window.ms = { // 创建一个全局对象
      set: set,
      get: get,
    };
  
    function set(key, val) { // 存储数据
      localStorage.setItem(key, JSON.stringify(val));
    }
  
    function get(key) { // 获取数据
  ...
  ;(function () { // ; 确保合并不会出错
    ...
    var Event = new Vue(); // 创建一个事件中心
  
    var alert_sound = document.getElementById('alert-sound'); // 提示音
